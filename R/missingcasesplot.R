    ...
    font.unit = "px")
{
    ...
    if (show.counts.missing || show.percentages.missing)
    {
        ...
        if (show.counts.missing)
            x.labels <- paste0(x.labels, SumColumns(dat, remove.missing = FALSE), " cases ")
        if (show.counts.missing && show.percentages.missing)
            x.labels <- paste(x.labels, "or ")
        if (show.percentages.missing)
            x.labels <- paste0(x.labels, round(SumColumns(dat, remove.missing = FALSE)/nrow(dat)*100), "%")
    ...
    {
        if (max(SumColumns(dat, remove.missing = FALSE)) > 1000)
            fill.opacity <- 0.2
        else if (max(SumColumns(dat, remove.missing = FALSE)) > 200)
            fill.opacity <- 0.5
        else
            fill.opacity <- 1.0
    }
