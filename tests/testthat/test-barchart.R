...
{
    print(Chart(y = good.examples[[i]]$Y, x = good.examples[[i]]$X, type = "Bar", transpose = good.examples[[i]]$transpose, title = names(good.examples)[i], aggregate.period = good.examples[[i]]$aggregate.period, hover.include.source.value = TRUE))
...
Chart(type = "Bar",
              ...
              y.grid.width = 0,
              x.grid.width = 1,
              x.zero.line.color = rgb(100, 100, 100, maxColorValue = 255),
              x.line.color = rgb(64, 64, 64, maxColorValue = 255),
              y.line.color = rgb(64, 64, 64, maxColorValue = 255),
              x.grid.color = rgb(100, 100, 100, maxColorValue = 255),
              bar.gap = 0,
              x.tick.format.manual = "$",
